...
def output_as(data, parameters):
    ...
    output_path = parameters['folder_path'] + '/' + parameters.get('output_path', 'Output/output.xlsx')
    ...
    with pd.ExcelWriter(output_path+'/'+file_name, engine='openpyxl') as writer:
        ...
        for sheet, sheet_data  in data.items():
            ...
            if isinstance(sheet_data, pd.DataFrame):
                ...
                pd.DataFrame(sheet_data).to_excel(writer, sheet_name=safe_sheet, index=False)
